...
class ModularFieldInteger:
    ...
    def __sub__(self, other: ModularFieldInteger) -> ModularFieldInteger:
        ...
        value = (self.value % self.prime - other.value % self.prime) % self.prime
        return self.__class__(value, self.prime)
    ...
    def __truediv__(self, other: ModularFieldInteger) -> ModularFieldInteger:
        ...
        #value = self.value * pow(a, self.prime - 2, self.prime) % self.prime
...
class S256Point(EllipcCurvePoint[ModularFieldInteger]):
    ...
    def parse(cls, sec_bin: bytes) -> S256Point:
        ...
        if sec_bin[0] == 4:
            return S256Point(x=int.from_bytes(sec_bin[1:33], "big"), y=int.from_bytes(sec_bin[33:65], "big"))
        is_even = sec_bin[0] == 2
        x = S256Integer(int.from_bytes(sec_bin[1:], "big"))
        # equation : y^2 = x^3 + 7
        right_side_equation = x ** 3 + S256Integer(EC_B)
        left_side_equation = right_side_equation.sqrt()
        if left_side_equation.value % 2 == 0:
            even_beta = left_side_equation
            odd_beta = S256Integer(EC_PRIME - left_side_equation.value)
        else:
            even_beta = S256Integer(EC_PRIME - left_side_equation.value)
            odd_beta = left_side_equation
        if is_even:
